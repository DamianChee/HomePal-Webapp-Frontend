...
import { getStatusColor, calculateTimeRemaining, calculatePauseEndTime, filterEvents, filterHistoryEvents } from './utils';
...
function MobileMonitorDashboard() {
  ...
  const [currentPages, setCurrentPages] = useState({ 'Today': 1, 'Yesterday': 1, 'Past Week': 1 }); // Separate pagination for each date
  ...
  const handleToggleExpand = (date) => {
    if (expandedDates.includes(date)) {
      ...
      console.log(`Showing more events for ${date}. Current page: ${currentPages[date]}`);
    }
  };
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-auto">
              ...
              {(() => {
                ...
                const filteredEvents = filterHistoryEvents(events, historyDateFilter, activeFilters);
